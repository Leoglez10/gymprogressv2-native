...
export default function ExerciseLibraryScreen() {
  ...
  // Nota: Ahora exerciseService.getAllExercises() ya devuelve la lista combinada
  // y deduplicada. Pero aquí lo reconstruimos para mantener la reactividad local
  // sin tener que recargar todo el tiempo del servicio.

  const allExercises = useMemo(() => {
    // 1. Obtener base de ejercicios (usando el servicio para obtener los defaults)
    // Para simplificar y asegurar consistencia, usamos una estrategia híbrida:
    // Los custom vienen de userExercises, los default los obtenemos del servicio
    // pero les aplicamos el estado local.

    // Sin embargo, para garantizar que NO HAYA DUPLICADOS visuales,
    // lo mejor es confiar en que la carga inicial (loadData) trajo todo,
    // y aquí solo filtramos/ordenamos.

    // Pero loadData separa en userExercises y "system".
    // Vamos a reconstruir usando la misma lógica que el servicio para estar seguros.

    // Importamos DEFAULT_EXERCISES de exerciseService para la base
    const { DEFAULT_EXERCISES } = require('../../services/exercises');

    const systemWithState = (DEFAULT_EXERCISES as Exercise[]).map(ex => ({
      ...ex,
    ...
    const combined = [...userExercises, ...systemWithState];
    const uniqueMap = new Map();
    combined.forEach(ex => uniqueMap.set(ex.id, ex));
